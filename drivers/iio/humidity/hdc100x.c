...
#define DEBUG_ON	0
#define HDC_DEBUG(fmt,arg...)          do{\
                                         if(DEBUG_ON)\
                                         printk("<<-HDC-DEBUG->> [%d]"fmt"\n",__LINE__, ##arg);\
                                       }while(0)
				   ...
				   struct iio_chan_spec const *chan)
{
	...
	HDC_DEBUG("The Value is 0x%x",val);
	HDC_DEBUG("The BE_Value is 0x%x",be16_to_cpu(val));
