...
void gtp_reset_guitar(struct i2c_client *client, s32 ms)
{
	...
	GTP_DEBUG("gtp_rst_gpio value %d\n", GTP_GPIO_GET_VALUE(gtp_rst_gpio));
    msleep(ms);                         // T2: > 10ms
...
static void gtp_parse_dt(struct device *dev)
{
	...
	GTP_INFO("******gtp_int_gpio value is %d\n",gtp_int_gpio);
	gtp_rst_gpio = of_get_named_gpio(np, "goodix,rst-gpio", 0);
	GTP_INFO("******gtp_rst_gpio value is %d\n",gtp_rst_gpio);
